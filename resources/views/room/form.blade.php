...
@section('content')
    ...
    <div class="card mb-4 p-3">
        <div class="card-body">
            <form
                @if ($data['type'] == 'Create') action="{{ route('rooms.store') }}" method="POST" @elseif($data['type'] == 'Edit') action="{{ route('rooms.update', $room->id) }}" method="POST"@else action="" method="" @endif>
                ...
                <div class="row row-cols-1 row-cols-md-2 g-4 mb-3">
                    <div class="col">
                        ...
                        <input type="text" class="form-control @error('name') is-invalid @enderror" id="name"
                            name="name"
                            @if ($room) value="{{ $room->name }}" @else value="{{ old('name') }}" @endif
                            autofocus>
                        @error('name')
                    ...
                    <div class="col">
                        ...
                        <input type="text" class="form-control @error('code') is-invalid @enderror" id="code"
                            name="code"
                            @if ($room) value="{{ $room->code }}" @else value="{{ old('code') }}" @endif
                            autofocus>
                        @error('code')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
